(function () {
    ...
    INC.config.clientToken = "d3R7m@Dr";
    INC.config.trackingURL = "https://usaoptimizedby.increasingly.co/ImportData";
    INC.config.addToBasketURL = "";
    INC.clientConfig.checkoutLink = "https://www.dermadoctor.com/checkout/cart/?nothanks=1";
    INC.config.affiliateURL = "";
    INC.config.pageType = "";
    INC.config.ivid = "";
    INC.config.pdpaddedProductList = [];
    INC.config.orignalPDPArray = []
    INC.config.rc1 = false
    ...
    INC.config.modalOffset = 0
    INC.config.currentQty = null;
    ...
    INC.config.noimage = "https://www.increasingly.co/Clients/d3R7m@Dr/images/no-image.png";
    ...
    INC.init = function () {
        ...
        switch (INC.config.pageType) {
            case "pdp":
                ...
                let clientATC = document.querySelector(".product-info-main");
                if (clientATC != null) {
                    if (document.querySelector('.product-info-main .product.alert.stock')) {
            ...
            case "productList":
                ...
                Array.prototype.forEach.call(document.querySelectorAll('.action.tocart'), function (atcButton) {
                    atcButton.addEventListener('click', function (e) {
                        INC.config.datastoreprod = [];
                        let qty = 1
                        let prdId = this.parentElement.dataset.productSku
                        let clientPrdBlock = this.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement
                        INC.config.plpProductId = prdId
                        let p_name = clientPrdBlock.querySelector('.product-item-name').innerText
                        let p_img = clientPrdBlock.querySelector('.product-image-photo').src
                        let p_activePrice = ""
                        let regularPrice = ""
                        let activepriceClnt = clientPrdBlock.querySelector('.price').innerText.replace("$", "");
                        let pdpprice = 0
                        if (clientPrdBlock.querySelector('.old-price')) {
                            pdpprice = clientPrdBlock.querySelector('.old-price').innerText.replace("$", "")
                        }
                        if (pdpprice == 0) {
                            pdpprice = undefined;
                        }
                        p_activePrice = activepriceClnt
                        regularPrice = pdpprice
                        INC.config.empty_prod_details = []
                        INC.config.empty_prod_details.push({
                            p_id: prdId,
                            p_name: p_name,
                            p_img: p_img,
                            p_activePrice: Number(p_activePrice),
                            regularPrice: Number(regularPrice)
                        })
                        let openSidebar = setInterval(() => {
                            let clientButton = atcButton
                            if (clientButton.classList.contains('disabled')) {
                                // Continue
                            } else {
                                INC.methods.irbReq(prdId, 30, "catalog_product_view");
    ...
    function cartpageprodids() {
        INC.config.cartproductids = [];
        let cartPageItems = checkoutConfig.quoteItemData
        cartPageItems.map((item) => INC.config.cartproductids.push(item.sku))
    ...
    INC.dataStore.methods = function () {
        ...
        function getProductByAtt(findObj) {
            ...
            for (let childProductId in mainProductListObj) {
                ...
                } else if (findObj.zeroText != "" && findObj.sizeText != "" && findObj.colorText != "" && mainProductListObj[childProductId]["option"][
                    "0"
                ] != undefined && mainProductListObj[childProductId]["option"]["1"] != undefined && mainProductListObj[childProductId]["option"][
                "2"] != undefined) {
                    if (findObj.colorText == mainProductListObj[childProductId]["option"]["0"].text && findObj.zeroText == mainProductListObj[
                        childProductId]["option"]["1"].text && findObj.sizeText == mainProductListObj[childProductId]["option"]["2"].text) {
                        childProductObj = mainProductListObj[childProductId];
                        return childProductObj;
                    }
    ...
    INC.classes.ProductObj = function (product) {
        ...
        this.maxQty = product != null && product != undefined ? product["Field5"] : "";
        ...
        this.wasPrice = product['Field2'] == "" ? "" : product['Field2']
                                ...
                                if (INC.config.empty_prod_details[0] != undefined) {
                                    if (INC.config.empty_prod_details[0].p_activePrice != undefined) {
                                        if (subtotalinc_active != null) {
                                            ...
                                            if (INC.config.empty_prod_details[0].regularPrice != undefined && !isNaN(INC.config
                                                .empty_prod_details[0].regularPrice)) {
                                        ...
                                        if (INC.config.empty_prod_details[0].p_name != undefined) {
                                            prd_names.innerHTML = "1X " + INC.config.empty_prod_details[0].p_name;
                                        }
                                        if (INC.config.empty_prod_details[0].p_name != undefined) {
    ...
    INC.methods.getProductIdFromWebPage = function () {
        let PRODUCTID = document.querySelector('#product_addtocart_form').dataset.productSku
        INC.config.Product_id = PRODUCTID;
        return PRODUCTID;
    };
    INC.methods.detectPageType = function () {
        if (document.querySelector(".catalog-product-view") != null) return "pdp";
        if (document.querySelector(".checkout-cart-index ") != null) return "cartPage";
        if (document.querySelector(".page-with-filter") != null) return "productList";
        if (document.querySelector(".catalog-category-view") != null) return "productList";
    };
    INC.methods.showSidebar = function (recsType) {
        console.log(recsType)
        ...
        if (sidebarel != null) {
            if (document.querySelector(".inc_recommended_tabs_list_item_block") == null || INC.config.sidebarFirstLoad == falseflag) {
                setTimeout(() => {
                    window.location.replace("https://www.dermadoctor.com/checkout/cart/?nothanks=1");
                }, 1000);
                return;
            }
    ...
    INC.methods.createAMBlock = function (PDP_type) {
        if (INC.dataStore.dataStoreObj.undefined == undefined) {
            ...
            if (PDP_type == "PDP") {
                ...
                if (document.querySelector('.inc_pdp_block')) {
                    document.body.classList.add('inc_fbt_af_block')
                }
                ...
                let countBlockAdded = 0;
                console.log(INC.config.cartproductids)
                if (bundleProductsArray.length > 0) {
                    bundleProductsArray.forEach(function (mainProductId) {
                        if (duplamid.indexOf(mainProductId) == -1) {
                            duplamid.push(mainProductId);
                            console.log(mainProductId)
                            if (!INC.config.cartproductids.includes((mainProductId))) {
                                let firstProductObjByMainId = INC.dataStore.methods().getFirstProductObjByMainId({
                                    mainId: mainProductId,
                                });
            ...
            if (PDP_type == "PDP") {
                let pdpBlockSibling = document.querySelector(".product-custom-description-attribute");
                if (window.innerWidth < 1024) {
                    pdpBlockSibling = document.querySelector('.product.attribute.overview')
                }
                ...
                if (pdpBlockSibling != null) {
                    ...
                    pdpBlockSibling = document.querySelector('.product-tabs-and-video-wrapper')
                    pdpBlockSibling = document.querySelector('.product-custom-description-attribute')
                    pdpBlockSibling.parentNode.insertBefore(amBlock, pdpBlockSibling);
                }
            } else {
                if (bundleProductsArray.length > 0) {
                    let CartPos = document.querySelector(".cart-discount");
                    // if (CartPos != null) {
                    //     CartPos.parentNode.insertBefore(amBlock, CartPos);
                    // }
                    insertAfter(CartPos, amBlock)
                }
            }
                ...
                for (let all_list in all_list_el) {
                    if (Object.prototype.hasOwnProperty.call(all_list_el, all_list)) {
                        ...
                        if (all_list_el[all_list].querySelector(".inc_product_desc_att_zero_block.inc_active") != null) {
                            if (prd_moduleb.querySelector('.inc_product_desc_att_size_list_item_img_block[data-cpid="' + main_idsd + '"]') !=
                                null) {
                                if (prd_moduleb.querySelector('.inc_product_desc_att_size_list_item_img_block[data-cpid="' + main_idsd +
                                    '"] .inc_product_desc_att_size_list_item_img_block') != null) {
                                    prd_moduleb.querySelector('.inc_product_desc_att_size_list_item_img_block[data-cpid="' + main_idsd +
                                        '"] .inc_product_desc_att_size_list_item_img_block').click();
                        ...
                        if (all_list_el[all_list].querySelector(".inc_product_desc_att_color_block.inc_active") != null) {
                            if (prd_moduleb.querySelector('.inc_product_desc_att_color_list_item_img_block[data-cpid="' + main_idsd + '"]') !=
                                null) {
                                if (prd_moduleb.querySelector('.inc_product_desc_att_color_list_item_img_block[data-cpid="' + main_idsd +
                                    '"]') != null) {
                                    prd_moduleb.querySelector('.inc_product_desc_att_color_list_item_img_block[data-cpid="' + main_idsd + '"]')
                                        .click();
                                }
        ...
        let allClientRemoveButton = document.querySelectorAll("#checkout-cart-table .cart-item__delete");
        for (const element of allClientRemoveButton) {
            element.addEventListener("click", function () {
                let client_popup = setInterval(() => {
                    let cartPageClientRemove = document.querySelector(".core_button_prev.add_to_cart_continue");
                    if (cartPageClientRemove) {
                        clearInterval(client_popup);
                        cartPageClientRemove.addEventListener("click", function () {
                            setTimeout(() => {
                                INC.clientConfig.offset = 0;
                                INC.clientConfig.slidevalue = 0;
                                INC.init();
    ...
    function addprodsingle(element) {
        ...
        if (productBlockNode.className == "inc_product_header_parent_mobile") {
            productBlockNode = productBlockNode.parentElement
        ...
        let add_btn = productBlockNode.querySelector(".inc_product_variant_simple_text");
        let add_btn_click = productBlockNode.querySelector(".inc_product_variant_simple_text");
        ...
        if (flag_check == 1) {
            if (INC.config.disablebtn == trueflag) {
                ...
                if (element.parentNode.parentNode.parentNode.parentNode.parentNode.classList.contains("inc_product_showcase_block") == trueflag || element
                    ...
                    if (mobile) {
                        productBlockNode.parentElement.parentElement.querySelector(".inc_product_variant_simple_text").innerText = "Adding...";
                    } else {
                    ...
                    INC.methods.addProductToCart(findObj, productBlockNode, add_btn_click, add_prod_im, add_btn, popbtn, rec, qty);
                }
            }
        }
    }
    ...
    INC.methods.createPDPBlock = function (sidebar) {
        ...
        if (sidebar != "sidebar" || sidebar == "hidesidebar") {
            if (INC.dataStore.dataStoreObj.undefined != undefined || empty_sidebarflag == falseflag) {
                ...
                if (INC.config.orignalPDPArray.length == 0) {
                    INC.config.orignalPDPArray = [...bundleProductsArray]
                } else if (INC.config.orignalPDPArray.length < bundleProductsArray.length) {
                    bundleProductsArray = INC.config.orignalPDPArray
                }
                ...
                if (pdpBlock.querySelectorAll(".inc_pdp_bundle_product_list_main_block .inc_product_img_main_img img").length > 2) {
                    ...
                    toplevelbundleblock = "<div class='inc_bundle_avail_img first'><img src='" + mainprodimgsrc +
                        ...
                        "'></div><div class='inc_bundle_avail_btn'>BUNDLE AVAILABLE</div>";
                } else {
                    inc_bundle_avail_block.classList.add("INCtwoProducts");
                    toplevelbundleblock = "<div class='inc_bundle_avail_img first'><img src='" + mainprodimgsrc +
                        "'></div><span></span> <div class='inc_bundle_avail_img third'><img src='" + secbundleimgsrc +
                        "'></div><div class='inc_bundle_avail_btn'>Bundle available</div>";
                }
                ...
                let pdpbundleel = document.querySelector(".prod_det_fields.left.product-section.product-section--add-to-cart");
                if (window.innerWidth < 769) {
                    pdpbundleel = document.querySelector(".prod_det_fields.left.product-section.product-section--add-to-cart");
                }
                INC.methods.clear();
                ...
                let pdpBlockSibling = document.querySelector(".prod_det_fields.left.product-section.product-section--add-to-cart");
                let pdpplacement = document.querySelector(".product-custom-description-attribute");
                if (pdpplacement == null) {
                    pdpplacement = document.querySelector('.product-tabs-and-video-wrapper')
                }
                if (window.innerWidth < 1024) {
                    pdpplacement = document.querySelector('.product.attribute.overview')
                }
                document.querySelector('.product-custom-description-attribute')
                ...
                if (document.querySelectorAll(".product-wrapper .product-form__item .image-radio input") != null) {
                    ...
                    for (const element of swatch1) {
                        if (element.checked) {
                            client_var_size = element.value.toLowerCase();
                            break;
                        }
                    }
                }
                if (document.querySelectorAll(".product-wrapper .product-form__item.colors .image-radio") != null) {
                    ...
                    for (const element of swatch2) {
                        if (element.classList.contains("current")) {
                            client_var = element.querySelector("input").value.replace("-", " ");
                            break;
                        }
                    }
                }
                ...
                if (document.querySelector(".product-add-form .swatch-option") != null && client_var == "") {
                    if (document.querySelector(".product-add-form .swatch-option").getAttribute("data-option-label") != null) {
                        client_var = document.querySelector(".product-add-form .swatch-option").getAttribute("data-option-label").trim()
                            .toLocaleLowerCase();
                    } else if (document.querySelector(".page-product-bundle .fieldset-bundle-options .option .option+.label") != null) {
                        client_var = document.querySelector(".page-product-bundle .fieldset-bundle-options .option .option+.label").parentNode.innerText
                            .trim().toLocaleLowerCase();
                    }
                }
                if (pdpBlock.querySelector(
                    ...
                    client_var_size + '"]') != null) {
                    pdpBlock.querySelector(
                        '.inc_pdp_product-main_block .inc_product_showcase_block .inc_product_desc_att_zero_block .inc_product_desc_att_size_list_item_img_block[datasize="' +
                        client_var_size + '"]').click();
                } else if (pdpBlock.querySelector(
                    ".inc_pdp_product-main_block .inc_product_showcase_block .inc_product_desc_att_zero_block .inc_product_desc_att_size_list_item_img_block"
                ) != undefined) {
                    pdpBlock.querySelector(
                        ".inc_pdp_product-main_block .inc_product_showcase_block .inc_product_desc_att_zero_block .inc_product_desc_att_size_list_item_img_block"
                    ).click();
                ...
                if (pdpBlock.querySelector(
                    '.inc_pdp_product-main_block .inc_product_modal_block .inc_product_desc_att_size_block .inc_product_desc_att_size_list_item_img_block[datasize="' +
                    client_var + '"]') != null) {
                    pdpBlock.querySelector(
                        '.inc_pdp_product-main_block .inc_product_modal_block .inc_product_desc_att_size_block .inc_product_desc_att_size_list_item_img_block[datasize="' +
                        client_var + '"]').click();
                ...
                for (let li in el_all_list_main1) {
                    if (el_all_list_main1[li] != null && Object.prototype.hasOwnProperty.call(el_all_list_main1, li)) {
                        if (el_all_list_main1[li].querySelector(
                            '.inc_product_desc_att_color_block .inc_product_desc_att_color_list_item_img_block[datacolor="' + client_var + '"] div'
                        ) != null) {
                            el_all_list_main1[li].querySelector(
                                '.inc_product_desc_att_color_block .inc_product_desc_att_color_list_item_img_block[datacolor="' + client_var +
                                '"] div').click();
                        } else if (el_all_list_main1[li].querySelector(
                            ".inc_product_desc_att_color_block .inc_product_desc_att_color_list_item_img_block div") != null) {
                            el_all_list_main1[li].querySelector(".inc_product_desc_att_color_block .inc_product_desc_att_color_list_item_img_block div")
                                .click();
                        }
                    }
                }
                ...
                if (pdpplacement == null && pdpBlockSibling == null) {
                    pdpplacement = document.querySelector('.product-info-main')
                    insertAfter(pdpplacement, pdpBlock);
                }
                ...
                setTimeout(function () {
                        ...
                        if (Object.prototype.hasOwnProperty.call(all_list_el, ell)) {
                            ...
                            if (all_list_el[ell] != null) {
                                if (all_list_el[ell].querySelector(".inc_product_desc_att_zero_block.inc_active") != null) {
                                    ...
                                    } else if (all_list_el[ell].querySelector('.inc_product_desc_att_size_list_item_img_block[datasize="' +
                                        client_var + '"] div') != undefined) {
                                        all_list_el[ell].querySelector('.inc_product_desc_att_size_list_item_img_block[datasize="' +
                                            client_var + '"] div').click();
                                ...
                                if (all_list_el[ell].querySelector(".inc_product_desc_att_color_block.inc_active") != null) {
                                    if (prd_moduleb.querySelector('.inc_product_desc_att_color_list_item_img_block[data-cpid="' + main_idsd +
                                        '"]') != null) {
                                        if (prd_moduleb.querySelector('.inc_product_desc_att_color_list_item_img_block[data-cpid="' +
                                            main_idsd + '"]') != null) {
                                            prd_moduleb.querySelector('.inc_product_desc_att_color_list_item_img_block[data-cpid="' +
                                                main_idsd + '"]').click();
                                        }
                                    } else if (all_list_el[ell].querySelector('.inc_product_desc_att_color_list_item_img_block[datacolor="' +
                                        client_var + '"] div') != undefined) {
                                        all_list_el[ell].querySelector('.inc_product_desc_att_color_list_item_img_block[datacolor="' +
                                            client_var + '"] div').click();
                    ...
                    if (document.querySelector(".inc_bundle_avail_block img") != null && document.querySelector(
                        ".inc_pdp_product-main_block .inc_product_img_block img") != null) {
                        document.querySelector(".inc_bundle_avail_block img").src = document.querySelector(
                            ".inc_pdp_product-main_block .inc_product_img_block img").getAttribute("data-src");
                    }
                    if (document.querySelector(".inc_bundle_avail_img.second img") != undefined && document.querySelectorAll(
                        ".inc_pdp_bundle_product_list_main_block .inc_product_showcase_block .inc_product_img_main_img img")[0] !=
                        undefined) {
                        let secimg = document.querySelectorAll(
                            ".inc_pdp_bundle_product_list_main_block .inc_product_showcase_block .inc_product_img_main_img img")[0].src;
                        document.querySelector(".inc_bundle_avail_img.second img").src = secimg;
                    }
                    if (document.querySelector(".inc_bundle_avail_img.third img") != undefined && document.querySelectorAll(
                        ".inc_pdp_bundle_product_list_main_block .inc_product_showcase_block .inc_product_img_main_img img")[1] !=
                        undefined) {
                        let thirdimg_ = document.querySelectorAll(
                            ".inc_pdp_bundle_product_list_main_block .inc_product_showcase_block .inc_product_img_main_img img")[1].src;
        ...
        document.querySelector('#product-addtocart-button').addEventListener('click', function () {
            ...
            let elPDP = document.querySelector(".product-info-main");
            INC.config.pdpaddedProductList = [];
            ...
            let openSidebar = setInterval(() => {
                let clientButton = document.querySelector('#product-addtocart-button')
                if (clientButton.classList.contains('disabled')) {
                    // Continue
                } else {
                    let allowed = JSON.parse(document.querySelector('.box-tocart input').dataset.validate)
                    let validations = allowed["validate-item-quantity"]
                    if (Number(document.querySelector('.box-tocart input').value) >= validations.minAllowed && Number(document
                        .querySelector('.box-tocart input').value) <= validations.maxAllowed) {
    ...
    INC.methods.updateSidebarBlock = function (mainord, elprd) {
        ...
        if (sbBlock != null) {
            ...
            bundleCartProductsArray.forEach(function (productSKU) {
                ...
                if (INC.clientConfig.OOS.indexOf(productObj.childsku) >= 0) {
                    if (oosname.indexOf(productObj.name) == -1 && oosids.indexOf(productObj.childsku) == -1) {
                        error_message = "";
                    ...
                    let addedhtml =
                        ...
                        productObj.imageURL + '" data-src="' + productObj.imageURL + '" alt="' + productObj.name +
                        '"><div class="sidebar_product_quantity_label">' + qty +
                        ...
                        "<span>" + qty + "x </span> " + productObj.name +
                ...
                if (all_list_el[le] != null && Object.prototype.hasOwnProperty.call(all_list_el, le)) {
                    ...
                    if (all_list_el[le].querySelector(".inc_product_desc_att_color_block.inc_active") != null) {
                        if (prd_moduleb.querySelector('.inc_product_desc_att_color_list_item_img_block[data-cpid="' + main_idsd + '"]') !=
                            null) {
                            if (prd_moduleb.querySelector('.inc_product_desc_att_color_list_item_img_block[data-cpid="' + main_idsd +
                                '"] .inc_product_desc_att_color_list_item_img_block') != null) {
                                prd_moduleb.querySelector('.inc_product_desc_att_color_list_item_img_block[data-cpid="' + main_idsd +
                                    '"] .inc_product_desc_att_color_list_item_img_block').click();
                            }
        ...
        if (flag_check == 1) {
            if (element.parentNode.parentNode.parentNode.parentNode.parentNode.classList.contains("inc_product_showcase_block") == trueflag || element
                ...
                if (INC.config.disablebtn == trueflag) {
                    ...
                    INC.methods.addProductToCart(findObj, productModuleBlock, btnclick, add_prod_im, selbtn, newbtntext, rec_, qty);
                }
            }
        }
    }
    INC.methods.addProductToSidebarCart = function (findObj, client_add_btn, _productBlockel, btn_addtocart, elmparent) {
        ...
        if (client_add_btn == "client_add_btn") {
            if (INC.config.pageType == "pdp") {
                if (elmparent.querySelector("#qty") != null) {
                    qty = elmparent.querySelector("#qty").value;
                }
            }
        }
        let dataStore = INC.dataStore;
        ...
        if (sbBlock != null) {
            ...
            if (INC.clientConfig.OOS.indexOf(productObj.mainId) >= 0) {
                ...
                if (client_add_btn == "client_add_btn" && INC.config.pageType !== 'productList') {
                    activepriceClnt = elmparent.querySelector('.price').innerText;
                    if (elmparent.querySelector('.old-price')) {
                        pdpprice = elmparent.querySelector('.old-price').innerText
                    }
                    if (pdpprice == null) {
                        pdpprice = 0;
                    }
                } else if (client_add_btn == "client_add_btn" && INC.config.pageType === 'productList') {
                    elmparent = document.querySelector('form[data-product-sku="' + INC.dataStore.pdpProductId + '"]').parentElement.parentElement
                ...
                let addedhtml = '<div class="inc_cart_added_product_img_block"><div class="inc_cart_added_product_img"><img class="inc_lazy" src="' +
                    productObj.imageURL + '" data-src="' + productObj.imageURL + '"  alt="' + productObj.name +
                    '"><div class="sidebar_product_quantity_label">' + qty +
    ...
    function prepareqty(productBlock, blockType) {
        ...
        elQtyInput.addEventListener("keyup", function (event) {
            if (event.target.value.length == 0) {
                INC.config.currentQty = event.target;
                setTimeout(() => {
                    if (INC.config.currentQty !== null) {
                        if (Number(INC.config.currentQty.value) == 0) {
                            INC.config.currentQty.value = 1;
                            INC.config.currentQty = null;
                        }
                    }
                }, 1500);
            ...
            } else if (window.jQuery.inArray(event.keyCode, incallowedKeys) != -1 || (keyCode === 65 && (event.ctrlKey || event.metaKey)) || (
                keyCode >= 35 && keyCode <= 40)) {
                return;
            }
            let charValue = String.fromCharCode(keyCode),
                valid = /^\d+$/.test(Number(charValue));
            if (!valid) {
                // event.preventDefault()
        ...
        elQtyInput.addEventListener("focusout", function () {
            let elmt = this;
            setTimeout(function () {
                qtyupdateInc(elmt, productBlock, elAddText, true);
            }, 50);
        });

        function qtyupdateInc(_elm, prdblk, eladtext, focusout) {
    ...
    INC.methods.createProductShowcaseBlock = function (pdpProductId, blockType, _bundleProductListCheck, _parentblock, elProductShowcaseBlock, mainprod) {
        ...
        let productAddButton = productBlock.querySelector(".inc_product_add_block");
        let elQtyAddBtnBlock = productBlock.querySelector(".inc_product_desc_qty_add_btn_block");
        let elRrpLabelBlock = productBlock.querySelector(".inc_product_rrp_label_block");
        ...
        productLink.href = productObj.ur;
        ...
        productLink.addEventListener("click", function (e) {
            e.preventDefault();
            INC.config.modalOffset = document.body.getBoundingClientRect().top
            document.querySelector("body").classList.add("inc_overlay");
            this.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.querySelector(".inc_product_modal_block")
        ...
        elTitleText.addEventListener('click', function (e) {
            e.preventDefault();
            INC.config.modalOffset = document.body.getBoundingClientRect().top
            document.querySelector("body").classList.add("inc_overlay");
            this.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.querySelector(".inc_product_modal_block")
        ...
        let discountPerc = getDiscount(productObj.regularPrice, productObj.activePrice)
        if (activePrice != regularPrice && regularPrice != "" && regularPrice != null && regularPrice != 0) {
            ...
            if (blockType == "am") {
                elproductPriceLabel.innerHTML = "Save " + formatter.format(totalsaveprc) + "(" + discountPerc + "%)";
            } else {
            ...
            elPriceActiveText.innerHTML = formatter.format(regularPrice);
        ...
        if (productSizesObj.sizeArray.length > 0) {
            _parentblock.classList.add("variant_avail");
            productBlock.querySelector(".inc_product_desc_att_size_block .inc_product_desc_att_size_title_block").addEventListener("click", function (
                e) {
            ...
            if (
                ...
                if (this.innerText.trim() == "Add to Cart" || this.innerText.trim() == "SELECT") {
                    if (flag_check == 1 || flag_check == 3) {
                        ...
                        this.parentNode.parentNode.querySelector(".inc_product_desc_variant_popup_close_block").click();
                        productBlock.classList.remove("show_overlay");
                        if (this.textContent.trim() == "Add to Cart" || this.textContent.trim() == "SELECT" || this.textContent.trim() ==
                            "Add") {
                            this.parentNode.parentNode.parentNode.querySelector(".inc_product_desc_add_block").click();
                        }
                    } else {
        ...
        elselectoptaddbtn.addEventListener("click", function (e) {
            if (productObj.ProductType === "simple" && e.target.innerText !== "Add to Cart") {
                document.querySelector("body").classList.add("inc_overlay");
            ...
            if (document.querySelector(".inc_sidebar_modal_block.inc_active") == null) {
                ...
                if (elAddText.textContent == "Add") {
                    ...
                    if (modalProductBlock.querySelector(".inc_product_desc_add_text").textContent != "Update" && prod_card.querySelector(
                        ...
                        elAddText.textContent = "Add to bundle";
    ...
    INC.methods.createProductModalBlock = function (pdpProductId, blockType, _bundleProductListCheck, _parentblock, elmodalblock, mainprod, rec_type) {
        ...
        let elHeaderInfoBlck = prdmblock.querySelector(".inc_product_info_main_block");
        let elManufacturer = prdmblock.querySelector(".inc_product_header_manufacturer");
        ...
        let elproductextension = prdmblock.querySelector(".inc_product_extension_block");
        if (window.innerWidth < 820) {
            ...
            let parentForThree = generate_html_tag("div", "inc_product_header_parent_mobile");
            insertAfter(elproductextension, parentForThree)
            parentForThree.appendChild(elHeader)
            parentForThree.appendChild(elHeaderInfoBlck)
            parentForThree.appendChild(productDescriptionMainBlock)
        }
        elHeader.querySelector(".inc_product_header_img_block").addEventListener("click", function () {
            if (document.querySelector(".inc_sidebar_modal_block.inc_active") == null) {
                document.querySelector("body").classList.remove("inc_overlay");
            ...
            let scrollDiv = Math.abs(INC.config.modalOffset);
            window.scrollTo({
                top: scrollDiv
            });
        });
    ...
    function productzeroatt(elAtt0TitleText, elAtt0Block, product0Obj, elAtt0ListBlock, blockType) {
        ...
        if (window.innerWidth < 769) {
            ...
            if (product0Obj["att0Array"][0].label == "Leather" || product0Obj["att0Array"][0].label == "Colour" || product0Obj["att0Array"][0].label ==
                "Colour:" || product0Obj["att0Array"][0].label == "Fabric" || product0Obj["att0Array"][0].label == "Marble" || product0Obj["att0Array"][0]
                    .label == "Finish") {
                elAtt0Block.parentNode.parentNode.parentNode.classList.add("dropdownstyle");
            }
        }
    ...
    function productcoloratt(productColorsObj, productBlock, elAttColorListBlock, blockType, elAttColorBlock, elAttColorTitleText, bType) {
        ...
        if (productColorsObj.colorArray.length > 0) {
            ...
            if (productColorsObj["colorArray"][0].label == "Select Fabric") {
                ...
                let sortingdata = ["Color", "Size", "Linen", "Wool", "Duro Velvet", "Bouclé", "Textured Chenille", "Wilbur Alpaca Blend", "Stonewash Linen",
                    "Twisted Yarn Cotton Blend", "Brenta Leather", "Fermo Leather", "Romagna Leather", "Riso Bouclé", "Alpaca Velvet", "Bora Bora",
                    "Dolce", "Strange Loves",
                ];
                let result = [];
    ...
    INC.methods.mappvariants = function (obj, blocktype, type_of_variant) {
        if (obj.parentNode != undefined) {
            ...
            if (type_of_variant == "zero" || type_of_variant == "third" || type_of_variant == "color") {
                ...
                if (productblockmod.querySelector('.inc_product_desc_att_size_list_block .inc_product_desc_att_size_list_item_img_block[datacolor="' +
                    sizelowertext + '"]') != null) {
                    productblockmod.querySelector('.inc_product_desc_att_size_list_block .inc_product_desc_att_size_list_item_img_block[datacolor="' +
                        sizelowertext + '"]').classList.add("inc_active");
                    productblockshow.querySelector('.inc_product_desc_att_size_list_block .inc_product_desc_att_size_list_item_img_block[datacolor="' +
                        sizelowertext + '"]').classList.add("inc_active");
                }
            }
        ...
        if (Object.keys(productObj).length != 0) {
            ...
            if (productModuleBlock.querySelector(".inc_product_desc_att_size_title_text") != null && sel_attr_text != "") {
                ...
                if (modal_attr_block.querySelector(
                    ...
                    if (optionimage == "" || optionimage == undefined) {
                        document.querySelector(".inc_product_desc_att_color_list_item_block.inc_active .inc_product_desc_att_color_list_item_img_block")
            ...
            if (productBlockel.parentNode.classList.contains("inc_product_modal_block") || productBlockel.parentNode.className ==
                ...
                if (productBlock.querySelector(".inc_product_header_main_title_block") != null) {
                    elModalTitle = productBlock.querySelector(".inc_product_header_main_title_block");
                }
            }
        ...
        if (elPdpBundleCartBlock != null) {
            ...
            if (elPdpBundleCartBlock != null && elPdpBundleCartBlock != undefined) {
                ...
                if (totalProductCount <= 1) {
                    elBundleCartAddBtnTexts.textContent = "Add to Cart";
                    elBundleCartAddBtnCount.innerHTML = Object.keys(INC.dataStore.bundleCartProducts).length + " ITEM SELECTED";
                } else if (totalProductCount == 2) {
                    elBundleCartAddBtnTexts.textContent = "Add both to Cart";
                    elBundleCartAddBtnCount.innerHTML = Object.keys(INC.dataStore.bundleCartProducts).length + " ITEMS SELECTED";
    ...
    INC.methods.createBundleCartAddedProductBlock = function (productId, qty) {
        ...
        if (prod_module_elm != null) {
            ...
            let addedsumprodhtml =
                ...
                productObj.name + '"><div style="text-transform: unset;"><span>' + qty + " <small>x</small> </span>" + productObj.name +
    ...
    INC.methods.addProductToCart = function (findObjd, productBlock, add_btn_click, _add_prod_im, btn_addtocart, newbtntext, rec_type, qty) {
        if (INC.config.pageType != "cartPage" && INC.config.pageType != "pdp" && INC.config.pageType != "productList") {
            document.querySelector("body").classList.remove("inc_overlay");
        }
        ...
        if (INC.config.pageType == 'cartPage') {
            let cartPageURL =
                "https://www.dermadoctor.com/checkout/cart/add/uenc/aHR0cHM6Ly93d3cuZGVybWFkb2N0b3IuY29tL2twLWR1dHktZHVvLmh0bWw_dXRtX2NhbXBhaWduPUREU0tQS0lUJnV0bV9tZWRpdW09Y3NlJnV0bV9zb3VyY2U9Z29vZ2xlUFM%2C/product/" +
                productObj.mainsku
            addProductURL = cartPageURL
        } else {
            if (INC.config.pageType == "productList") {
                addProductURL = document.querySelector('form[data-role="tocart-form"]').action.split('/product/')[0] + '/product/' + productObj.mainsku
            } else {
                addProductURL = document.querySelector('.product-add-form form').action.split('/product/')[0] + '/product/' + productObj.mainsku
            }
        }
        let key = readCookie('form_key')
        let params = ""
        params =
            `product=${productObj.mainsku}&selected_configurable_option=&related_product=&item=${productObj.mainsku}&form_key=${key}&subscription_option[option]=onetime_purchase&subscription_option[interval]=Every 30 Days&qty=${qty}`

        let ATC_URL = addProductURL
        window.jQuery.ajax({
            type: "POST",
            url: ATC_URL,
            data: params,
            ...
            error: function (res) {
                ...
                if (document.querySelector('.inc_justerror') == null) {
                    let headertitle_ = document.querySelector('.inc_sidebar_modal_cart_and_title_block')
                    let errormes = generate_html_tag('div', 'inc_justerror')
                    let errormestext = generate_html_tag('div', 'inc_justerror_text')
                    errormestext.innerHTML = "<div><img style='width:40px' src='" + productObj.imageURL +
                        "'/> Something happend try again</span></div>"
                    errormes.appendChild(errormestext)
                    headertitle_.parentNode.insertBefore(errormes, headertitle_)
                    setTimeout(function () {
                        let remel__1 = document.querySelector('.inc_justerror')
                        if (remel__1 != null) {
                            remel__1.parentNode.removeChild(remel__1)
                        }
                    }, 2000)
                }
                if (INC.config.pageType != "cartPage") {
                ...
                setTimeout(function () {
                    if (document.getElementsByClassName("inc_sidebar_modal_block") != null) {
                        if (document.getElementsByClassName("inc_sidebar_modal_block")[0] != undefined) {
                            document.getElementsByClassName("inc_sidebar_modal_block")[0].scrollTop = 0
                        }
                    }
                }, 50)
            },
        ...
        if (rec_type == trueflag) {
            ...
            if (productObj.mainId != undefined) {
        ...
        function addsuccess_(btn_t, addbtnt, addedprdblock) {
            INC.config.disablebtn = true;
            addbtnt.innerText = "Added to Cart";
            btn_t.innerText = "Added to Cart";
            if (newbtntext != undefined) {
                ...
                setTimeout(function () {
                    ...
                    if (INC.config.pageType == "pdp" || INC.config.recommendation == trueflag || rec_type == trueflag) {
                        if (document.querySelector(".sidebar_outer.sidebar_outer_active") == null) {
                            ...
                            INC.methods.updateSidebarBlock(productObj.mainId, productBlock);
                            INC.methods.showSidebar(rec_type);
                        } else {
                            let allprd = addedprdblock.querySelectorAll(".inc_recommended_products_list_block .inc_product_module_block")
                                .length;
                            if (document.querySelector(".inc_recommended_products_list_block .inc_product_module_block") == null || (allprd ==
                                1 && document.querySelector(".sidebar_outer.sidebar_outer_active") != null)) {
                                if (document.querySelector(".inc_recommended_tabs_list_item_block.inc_active") != null) {
                                    ...
                                    if (document.querySelector(".inc_recommended_tabs_list_item_block.inc_active") != null) {
                                        document.querySelector(".inc_recommended_tabs_list_item_block.inc_active").parentNode.removeChild(
                                            remcat);
                                        ...
                                        if (document.querySelectorAll(".inc_recommended_tabs_list_item_block").length > 1) {
                                            ...
                                            checkTabEmpty();
                                            document.querySelector(".inc_recommended_products_list_block").setAttribute("style",
                                                "margin-left:0;");
                                        }
                                        if (window.innerWidth < 821) {
                                ...
                                if (addedprdblock.parentNode.querySelectorAll(".inc_recommended_products_list_block .inc_product_module_block")
                                    .length == 1) {
                                    let len = addedprdblock.parentNode.querySelectorAll(
                                        ".inc_recommended_products_list_block .inc_product_module_block").length;
                                    checkTabEmpty(len);
                                    if (document.querySelector(".inc_recommended_tabs_list_item_block") != null) {
                                        document.querySelector(".inc_recommended_tabs_list_item_block").click();
                                    }
    ...
    INC.methods.addBundleToCart = function (btn_cart) {
        ...
        Object.keys(dataStore.bundleCartProducts).forEach(function (bundleCartProduct) {
            ...
            let productObj = INC.dataStore.methods().getProductById(findObj);
            let addProductURL = document.querySelector('.product-add-form form').action.split('/product/')[0] + '/product/' + productObj.mainsku
            let key = document.querySelector('[name="form_key"]').value
            let params = ""
            params =
                `product=${productObj.mainsku}&selected_configurable_option=&related_product=&item=${productObj.mainsku}&form_key=${key}&subscription_option[option]=onetime_purchase&subscription_option[interval]=Every 30 Days&qty=${INC.dataStore['bundleCartProducts'][bundleCartProduct]}`
    ...
    function ajax_add_to_cart(formData, url_add, btn_cart, recsType, qtyArray) {
        INC.clientConfig.OOS = [];
        INC.clientConfig.OOS_MESG = [];

        function suc1(btn_ca, res) {
            if (formData[1] == undefined) {
                loadsidebarajax(btn_ca, false);
            }
            if (formData[1] != undefined) {
                callprodB();
            }
        ...
        function callprodA() {
            window.jQuery.ajax({
                ...
                success: function (data_re) {
                    if (Array.isArray(data_re)) {
                        suc1(btn_cart, data_re);
                    } else {
                        prod_qty_notinstock(formData[0])
                        suc1(btn_cart, data_re);
                    }
                },
                error: function (data_re) {
    ...
    function prod_qty_notinstock(formData) {
        INC.clientConfig.OOS = [];
        INC.clientConfig.OOS_MESG = [];
        let ajaxmainid = formData.split("product=")[1].split('&')[0]
        INC.clientConfig.OOS.push(ajaxmainid);
        INC.clientConfig.OOS_MESG.push('The requested qty exceeds the maximum qty allowed in shopping cart');
    }
    ...
    INC.methods.trackingEvents = function (eventType, eventDataObj, pageType, rec_type) {
        ...
        switch (eventType) {
            ...
            case "bundleProductClickTracking":
                ...
                if (
                    (INC.config.recommendation == trueflag && INC.config.recommendationProduct.includes(eventDataObj.product_id)) && INC.config.pageType ==
                    "pdp") {
    ...
    function clickListener(e) {
        ...
        if (clickedElement != null) {
            try {
                ...
                if (document.querySelector(".inc_product_modal_block.inc_active") != null) {
                    if (clickedElement.parentNode.classList == "inc_product_desc_att_color_list_item_img" || clickedElement.classList ==
                        "inc_product_desc_att_color_list_item_img" || clickedElement.parentNode.classList == "inc_product_desc_att_zero_list_block" ||
    ...
    function inc_showSlides(n, _ind, elm, _type_of, arrow) {
        if (elm.className !== "inc_product_module_block simple") {
    ...
    function incPriceMismatch(active_price, special_price) {
        let data = {};
        ...
        let elProductPrice = document.querySelector(".product-info-main .price");
        let elSpecialPrice = document.querySelector(".product-info-main .old-price");
        let clientSpecialPrice = elSpecialPrice ? Number(elProductPrice.innerText.replace(/[^0-9.-]+/g, "")) : null;
        let clientProductPrice = Number(elProductPrice.innerText.replace(/[^0-9.-]+/g, ""));
        if (clientSpecialPrice) {
            clientProductPrice = elSpecialPrice.innerText.replace(/[^0-9.-]+/g, "")
        }
        let final_product_price = Number(clientProductPrice);
        let final_special_price = Number(clientSpecialPrice);
        let final_old_product_price = Number(active_price);
        let final_old_client_special_price = (special_price) == 0 ? null : Number(special_price);
    ...
    function oosproduct(prdid) {
        ...
        if (product_id != null) {
            ...
            let p_url = "https://usagather.increasingly.com/ProductInventoryUpdate";
            let xhr = new XMLHttpRequest();
            xhr.open("POST", p_url, true);
            xhr.onreadystatechange = function () {
    ...
    function getDiscount(price, sale) {
        return parseFloat(((price - sale) / price) * 100).toFixed(2);
    }
    ...
    function inc_isOnScreen_page(elem) {
        ...
        if (isIOS) {
            ...
            if (el != null) {
                ...
                return (top < (window.pageYOffset + window.innerHeight + 2000) && left < (window.pageXOffset + window.innerWidth) && (top + height) > window
                    .pageYOffset && (left + width) > window.pageXOffset);
            }
        } else {
            if (elem.length == 0) {
                return;
            ...
            return (top >= viewport_top && top < viewport_bottom) || (bottom > viewport_top && bottom <= viewport_bottom) || (height > viewport_height &&
    ...
    function element_in_viewport(el) {
        ...
        return (top < (window.pageYOffset + window.innerHeight) && left < (window.pageXOffset + window.innerWidth) && (top + height) > window.pageYOffset &&
